...
Always explain what you observe and why it matters."""
    ...
    async def execute_task(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        print(
            f"  📊 Analyzing data: {data_info.get('n_rows', 0)} rows, {data_info.get('n_cols', 0)} columns"
        )
...
Be specific and prioritized."""
    ...
    async def _perform_eda(
        self, data_info: Dict[str, Any], eda_plan: List[str]
    ) -> EDAReport:
        ...
        return EDAReport(
            ...
            correlations={"x_y": 0.75, "x_z": 0.3},
            outliers_detected=[
                "Row 150: y value extreme",
                "Row 890: potential data entry error",
            ],
            missing_data_summary={"x": 0, "y": 5, "z": 12},
    ...
    async def _generate_visualizations(
        self, data_info: Dict[str, Any], eda_report: EDAReport
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_plot_code(
        self, plot_type: str, x: Optional[str] = None, y: Optional[str] = None
    ) -> str:
...
Always prioritize interpretability and statistical validity."""
    ...
    async def execute_task(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "agent": self.name,
            "status": "success",
            "result": {"model": fitted_model.model_dump(), "code": modeling_code},
...
"""
    ...
    async def _fit_model(self, spec: ModelSpec, code: str) -> FittedModel:
        ...
        return FittedModel(
            ...
            coefficients={"intercept": 10.5, "x1": 2.3, "x2": -1.7},
            statistics={
                "r_squared": 0.75,
                "adj_r_squared": 0.74,
                "f_statistic": 150.5,
...
Write for a business audience while maintaining statistical accuracy."""
    ...
    async def execute_task(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "agent": self.name,
            "status": "success",
            "result": {"interpretation": interpretation.model_dump(), "report": report},
    ...
    def _format_significance(self, sig_dict: Dict[str, Dict[str, Any]]) -> str:
        ...
        for var, stats in sig_dict.items():
            sig_marker = (
                ...
                else "**" if stats["p_value"] < 0.01 else "*"
            )
            lines.append(
                f"- **{var}**: p={stats['p_value']:.4f} {sig_marker} (Effect size: {stats['effect_size']})"
            )
        return "\n".join(lines)
...
async def demo_specialized_agents():
    ...
    print("-" * 70)
    eda_task = {"type": "eda", "input_data": data_info}
    ...
    modeling_task = {
        "type": "modeling",
        "input_data": {**data_info, "problem_type": "regression"},
    }
    ...
    print("-" * 70)
    interpretation_task = {"type": "interpretation", "input_data": {"model": model}}

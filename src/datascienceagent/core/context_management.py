...
class ContextType(str, Enum):
    ...
    TRANSFORMATION_NEEDS = "transformation_needs"
...
class ContextStore:
    ...
    def __init__(
        ...
        self.collection = self.client.get_or_create_collection(
            name=collection_name, metadata={"description": "Agent context and memory"}
        )

        logger.info(f"✅ Context store initialized: {persist_directory}")
    ...
    def add_chunk(self, chunk: ContextChunk):
        ...
        # Add to ChromaDB
        self.collection.add(
            documents=[chunk.content], metadatas=[metadata], ids=[chunk.id]
        )
    ...
    def query(self, query: RetrievalQuery) -> List[Dict[str, Any]]:
    ...
    def get_session_chunks(
        ...
        if context_types:
            where_filter["context_type"] = {"$in": [ct.value for ct in context_types]}

        results = self.collection.get(
            where=where_filter, limit=1000  # Adjust as needed
        )
    ...
    def get_topic_history(self, topic_id: str, limit: int = 50) -> List[Dict[str, Any]]:
        """Get historical context for a topic across sessions"""

        results = self.collection.get(where={"topic_id": topic_id}, limit=limit)
    ...
    def chunk_code(
        ...
        for i, line in enumerate(lines):
            ...
            should_chunk = (
                ...
                or (line.strip().startswith("def ") and current_size > 200)
                or (line.strip().startswith("class ") and current_size > 200)
                or i == len(lines) - 1
            )
    ...
    def chunk_agent_output(
        ...
        if "code" in output_dict:
            ...
            if isinstance(code, dict):
                ...
                chunks.extend(
                    self.chunk_code(code, "python", session_id, topic_id, agent_name)
                )
...
Focus on what's actionable and relevant for continuing the analysis.""",
    ...
    async def summarize_session(
        self, chunks: List[Dict[str, Any]], focus: Optional[str] = None
    ) -> str:
    ...
    def _extract_findings(self, chunks: List[Dict[str, Any]]) -> List[str]:
        ...
        for chunk in chunks:
            ...
            if metadata.get("context_type") in ["model_result", "agent_output"]:
                ...
                if any(
                    term in content for term in ["significant", "correlation", "effect"]
                ):
...
class ContextManager:
    ...
    def start_session(self, topic: Optional[str] = None) -> str:
        ...
        if self.current_topic_id:
            logger.info(f"   Topic: {self.current_topic_id}")
    ...
    def store_agent_thought(
        self, agent_name: str, thought: str, metadata: Optional[Dict[str, Any]] = None
    ):
    ...
    def store_agent_output(
        ...
        chunks = self.chunker.chunk_agent_output(
            output, agent_name, self.current_session_id, self.current_topic_id
        )
    ...
    async def get_context_for_agent(
        self, agent_name: str, current_task: str, include_history: bool = True
    ) -> ContextSummary:
        """Get relevant context summary for an agent"""

        if not self.current_session_id:
            return ContextSummary(session_summary="No active session", total_chunks=0)
        ...
        summary = await self.summarizer.summarize_for_agent(
            agent_name, current_task, session_chunks, history_chunks
        )
...
class ContextAwareAgent:
    ...
    async def execute_with_context(self, task: str, **kwargs) -> Dict[str, Any]:
    ...
    def _build_context_prompt(self, task: str, context: ContextSummary) -> str:
...
async def demo_context_system():
    ...
    context_mgr = ContextManager(
        persist_directory="./demo_chroma_db", chunk_size=500, model="ollama:llama3.2"
    )
...
""",
    ...
    context_mgr.store_user_query("Predict customer churn using logistic regression")
